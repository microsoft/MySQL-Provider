...
namespace Scx.Test.MySQL.Provider
{
    ...
    public class VerifyEnumerate : ISetup, IRun, IVerify, ICleanup
    {
        ...
        public void Setup(IContext mcfContext)
        {
            ...
            if (mcfContext.Records.HasKey("useNonSuperUser") &&
              mcfContext.Records.GetValue("useNonSuperUser") == "true")
            {
            ...
            this.username = mcfContext.ParentContext.Records.GetValue(usernamep);
            if (String.IsNullOrEmpty(this.username))
            {
                throw new VarAbort("username not specified");
            }

            this.password = mcfContext.ParentContext.Records.GetValue(passwordp);
